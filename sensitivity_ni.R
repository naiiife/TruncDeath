...
plot(2*(1:R)/R-1,bias.sc.s,type='l',lwd=2,ylim=c(-5,5),ylab='bias',
     xlab=expression(beta),main='Non-interaction')
points(2*(1:R)/R-1,bias.wzr.s,type='l',lwd=2,col='red')
points(2*(1:R)/R-1,bias.aipw.s,type='l',lwd=2,col='blue')
points(2*(1:R)/R-1,bias.reg.s,type='l',lwd=2,lty=2,col='blue')
abline(h=0,lty=2)
